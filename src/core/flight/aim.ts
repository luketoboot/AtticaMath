 ...
 * lock now: a challenger has to beat its bearing by `hysteresis` radians to
 * steal it, so two rocks straddling the nose do not strobe the highlight as
...
export function pickByNose(
  ...
  if (currentId !== null && currentGap !== null && bestId !== currentId) {
    if (currentGap - bestGap < hysteresis) return currentId;
  }
  return bestId;
}
